## ============================================================
##  BLRT for semiparametric model with Nadaraya–Watson
##  Dirichlet kernel (NWD), linear DGP (beta1 = 0)
## ============================================================

## ---- Load shared Dirichlet-kernel functions from GitHub (branch = R) ----
...
for (ai in seq_along(r1_seq)) {
  ...
  sim_res <- foreach(
    ...
    ## ---------- Semiparametric model M3 (Nadaraya–Watson Dirichlet) ----------
    data2 <- as.matrix(linear.data[, c("x1", "x2", "x3")])
    
    ## Bandwidth selection via CV.Best.h (NWD)
    h <- CV.Best.h(data2, y, alp)
    
    ## Nadaraya–Watson smoother matrix:
    ## S_ij = K(x_i, x_j) / sum_j K(x_i, x_j)
  ...
  path_ai <- file.path(
    base_path,
    paste0("power_result_線性係數=", round(r1, 2), "beta顯著.csv")
  )
...
## ============================================================
##  Read back results for each (r1, r2, r3) and summarize
## ============================================================
path_vec <- file.path(
  base_path,
  paste0("power_result_線性係數=", round(r1_seq, 2), "beta顯著.csv")
)
...
for (i in seq_along(r1_seq)) {
  ...
  ## Column 4 is Select (or data_i$Select)
  ...
  ## Column 5 is Beta1 (TRUE/FALSE); mean() is proportion significant
  result_list[[i]] <- c(
    full_tab,
    beta1 = mean(data_i[, 5])
  )
