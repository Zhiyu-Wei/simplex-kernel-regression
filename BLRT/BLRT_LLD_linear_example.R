## ============================================================
##  BLRT Simulation for Linear Data-Generating Mechanism (beta1 = 0)
##  Main script (uses functions in: BLRT_LLD_linear.R)
## ============================================================
...
B  <- 1000    # bootstrap replications in BLRT
N  <- 100     # sample size
alp <- 1      # Dirichlet kernel parameter (alpha)
beta1 <- 0    # true coefficient for z1
type1error <- 0.05  # significance level used in BLRT decisions
...
base_path <- file.path(getwd(), "BLRT_LLD_linear_results")
...
for (ai in seq_along(r1_seq)) {
  ...
  sim_res <- foreach(
    ...
    ## Simple seed inside each worker (for reproducibility)
    set.seed(123 + i)
    ...
    ## ---------- Semiparametric model M3 (Local linear Dirichlet) ----------
    ...
    X_kern_train <- data2
    X_loc_train  <- data2[, 1:2, drop = FALSE]
    ...
    ## Matrices shared between observed fit and bootstrap fits
    ...
    ## ---------- Bootstrap (within worker using a simple for loop) ----------
  ...
  ## ---------- Save raw results for this (r1, r2, r3) combination ----------
  path_ai <- file.path(
    base_path,
    paste0("power_result_線性係數=", round(r1, 2), "beta不顯著.csv")
  )
  write.csv(df, path_ai, row.names = FALSE, fileEncoding = "big5")
}

stopCluster(cl)


## ================== Output overall power_result ==================
print(power_result)
cat("Total elapsed time:", Sys.time() - total_start, "\n")

path_power <- file.path(
  base_path,
  "power_result線性係數.beta不顯著.csv"
)
...
path_vec <- file.path(
  base_path,
  paste0("power_result_線性係數=", round(r1_seq, 2), "beta不顯著.csv")
)
...
for (i in seq_along(r1_seq)) {
  ...
  ## Ensure all model labels appear (fill missing with 0)
