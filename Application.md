...
The linear component of the partial linear model indicates that neither institutional trust nor perceived corruption has a statistically significant effect on vaccine side effect concern. Although the estimated coefficient for perceived corruption is negative in our analysis, this direction should be interpreted with caution. Prior studies have generally documented a positive association, suggesting that higher perceived governmental corruption may undermine public confidence in health initiatives. One possible reason for this discrepancy is that the sample used in our study differs from those examined in earlier research. In particular, our dataset does not include several high-income, socio-politically diverse, and highly polarized countries—such as the United States—where perceptions of governmental integrity and vaccine attitudes are known to interact strongly. Combined with the limited sample size and the exclusion of several countries due to missingness, these differences highlight the need for further investigation using a more comprehensive dataset.

Turning to the nonparametric component, **Figure4** displays the estimated smooth surface $m(x)$ over the educational composition simplex. The plot reveals a nonlinear and non-monotonic relationship between education composition and concern levels. Elevated concern is observed in countries where the majority of the population holds a four-year degree but few possess postgraduate education. In contrast, concern tends to be lower in compositions dominated either by postgraduate education or by individuals with only secondary education.

These patterns suggest that vaccine concern is not attributable to any single level of education in isolation, but instead emerges from the relative balance among categories. This highlights the importance of accounting for interaction effects inherent in compositional data. The use of kernel smoothing provides a flexible and interpretable framework for uncovering such effects—patterns that would be obscured under a purely linear modeling approach.
